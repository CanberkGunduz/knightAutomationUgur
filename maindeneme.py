...
"""
1. Priestlerde MS'yi bage taşımadığı için satmaya calısıyor. Satılamaz yazısı çıkınca da bir daha işlem yapamıyor. Mecburen MS'leri çöpe atacağız veya çözüm sana kalmış.
2. Sundiresi görmüyor karakter infolarının olduğu yerden dolayı. Dün söylemiştin de hatırlayamadım nasıl düzelttiğimizi
3. MSleri taşıma işlemini yaparken arada sekmeler yaşanıyor.
"""
...
class KnightBot:
    ...
    def is_bosalt_typed(self):
        detector = self.detector
        img = ImageGrab.grab(bbox=(0, 620, 430, 700))
        img_cv2 = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
