...
class KnightBot:
        ...
        while True:
            ...
            if not self.is_genie_active():
                ...
                if self.is_bosalt_typed():
                    ...
                    self.open_inventory()
                    time.sleep(2)
                    self.locate_valuable_items_in_inv()
                    # time.sleep(3)
                    # self.sell_items()
    ...
    def is_genie_active(self):
        ...
        img = ImageGrab.grab(bbox=(668, 30, 848, 110))
        img_cv2 = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
        template = self.misc_images[1]
        detected, pos, located_precision = detector.locate_image_rgb(template, img_cv2, 0.92)
        return not detected

    def is_bosalt_typed(self):
        detector = self.detector
        img = ImageGrab.grab(bbox=(8, 650, 438, 730))
        img_cv2 = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
    ...
    def open_shop(self):
        self.press_esc()
        self.align_shop()
        self.mouse_click("left", 798, 730)
        self.mouse_click("right", 510, 530)
        time.sleep(2)
        try:
    ...
    def open_trade(self):
        ...
        time.sleep(2)
        self.mouse_click("left", 828, 340)
        self.mouse_click("left", 728, 580)
        p.moveTo(558, 530)
    ...
    def open_vip_inventory(self):
        try:
            self.open_inventory()
            self.mouse_click("left", 688, 370)
            time.sleep(2)
            p.locateOnScreen(
                "images/misc/vip_inventory_identifier.jpg",
                confidence=0.7,
                region=(638, 105, 988, 270)
            )
        except p.ImageNotFoundException:
            time.sleep(0.1)
            self.press_esc()
    ...
    def locate_items_in_inventory(self):
        ...
        while True:
            is_empty = p.pixelMatchesColor(
                688 + j * 49, 451 + i * 49, (25, 25, 25), 30
            )
            if not is_empty:
                item_coordinates.add((688 + j * 49, 470 + i * 49))
            else:
                empty_slots.add((688 + j * 49, 470 + i * 49))
            j += 1
            j = j % 7
            if j == 0:
                i += 1
    ...
    def locate_items_in_vip_inventory(self):
        ...
        while True:
            is_empty = p.pixelMatchesColor(
                328 + j * 51, 90 + i * 51, (25, 25, 25), 30
            )
            if not is_empty:
                item_coordinates.add((323 + j * 51, 110 + i * 51))
            else:
                empty_slots.add((323 + j * 51, 110 + i * 51))
            j += 1
            j = j % 6
            if j == 0:
                i += 1
    ...
    def deposit_all_to_vip(self):
        ...
        for i in range(2):
            for j in range(7):
                self.mouse_click("right", 673 + 50 * j, 410 + 50 * i)
        self.press_esc()

    def is_valuable_item_inv(self):
        detector = self.detector
        img = ImageGrab.grab(bbox=(298, 30, 1008, 550))
        img_cv2 = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
    ...
    def is_valuable_item_vip(self):
        detector = self.detector
        img = ImageGrab.grab(bbox=(8, 30, 628, 550))
        img_cv2 = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
        mp_detected, mp_pos, mp_located_precision = detector.locate_image_rgb(self.mp_text, img_cv2, 0.9)
        for template_index in range(len(self.weapon_images)):
            if template_index < 3:
        ...
        for item in items:
            ...
            if ms_detected:
                if len(bag2_empty_slots) > 0:
                    ...
                    self.mouse_drag("left", item[0], item[1], 978, 400)
                    self.mouse_click("left", 763, 425)

        self.mouse_click("left", 493, 410)

    def ts_scroll(self):
        self.mouse_click("left", 70, 475)
        self.mouse_click("left", 70, 475)
        time.sleep(1)
        self.mouse_click("left", 923, 145)
        self.mouse_click("left", 898, 575)
        time.sleep(1)
        self.mouse_click("left", 438, 470)
    ...
    def sell_items(self):
        ...
        while True:
            while True:
                ...
                time.sleep(0.4)
                self.mouse_click("left", 908, 490)
                time.sleep(0.9)
                self.mouse_click("left", 733, 580)
                time.sleep(0.4)
                self.mouse_click("left", 1008, 62)
                self.press_esc()
                time.sleep(2)
                self.open_inventory()
                time.sleep(2)
    ...
    def repair_items(self):
        ...
        for i in range(4):
            for j in range(3):
                self.mouse_click("right", 488 + 51 * j, 450 + 51 * i)
        time.sleep(2)
        self.open_repair()
        for _ in range(2):
            for i in range(2):
                for j in range(7):
                    self.mouse_click("left", 688 + j * 50, 470 + 50 * i)
            self.mouse_click("left", 688, 570)
            self.mouse_click("left", 878, 400)
            self.mouse_click("left", 928, 400)
            self.mouse_click("left", 928, 350)
            self.mouse_click("left", 928, 250)
            self.mouse_click("left", 928, 200)
            self.mouse_click("left", 878, 300)
        ...
        self.mouse_drag("left", 688, 470, 488, 450)
        self.mouse_drag("left", 728, 470, 538, 450)
        self.mouse_drag("left", 778, 470, 588, 450)

        self.mouse_drag("left", 828, 470, 488, 500)
        self.mouse_drag("left", 878, 470, 538, 500)
        self.mouse_drag("left", 928, 470, 588, 500)

        self.mouse_drag("left", 978, 470, 488, 550)
        self.mouse_drag("left", 688, 520, 538, 550)
        self.mouse_drag("left", 728, 520, 588, 550)

        self.mouse_drag("left", 778, 520, 488, 600)
        self.mouse_drag("left", 828, 520, 538, 600)
        self.mouse_drag("left", 878, 520, 588, 600)
...
class ImageDetector:
    ...
    def locate_image_rgb(self, template, img_cv2, precision=0.8):
